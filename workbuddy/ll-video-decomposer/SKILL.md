...
本 Skill 的脚本位于 Skill 目录的 `scripts/` 下，参考文档位于 `references/` 下。运行脚本时使用 WorkBuddy 管理的 Python 运行时（绝对路径，非裸 `python` 命令），脚本内部会自动发现 ffmpeg、ffprobe、yt-dlp 和各 Whisper 后端。

脚本之间的相对引用（如 `from tool_paths import ...`）依赖 Python 自动将脚本所在目录加入 `sys.path`，因此直接以绝对路径调用单个脚本即可正常工作，无需额外设置 `PYTHONPATH`。

首次处理完整视频前，通过 `scripts/setup_transcription.py --install` 在当前项目下创建独立虚拟环境 `.video-decomposer-venv`，不污染系统 Python。安装前须取得用户同意并说明下载体积。
